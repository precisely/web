...
export async function setDynamoTableThroughput(tableName: string, throughput: DynamoDB.ProvisionedThroughput) {
  const dynamo = <DynamoDB> new dynogels.AWS.DynamoDB();
  const currentThroughput = await getDynamoTableThroughput(tableName);
  if (
    currentThroughput.ReadCapacityUnits !== throughput.ReadCapacityUnits
    || 
    currentThroughput.WriteCapacityUnits !== throughput.WriteCapacityUnits
  ) {
