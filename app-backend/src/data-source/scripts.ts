...
export function deploy(environment: string) {
  ...
  shell.exec(`./node_modules/.bin/serverless decrypt -s ${environment}`, function(code: number, stdout: String) {
    ...
    if (position > -1) {
      ...
      shell.exec(`./node_modules/.bin/env-cmd ../config/security.env.dev ./node_modules/.bin/serverless deploy -s ${environment}`, function (deployCode, deployStdout) {
